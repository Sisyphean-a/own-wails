...
这是一个包含四个独立 Wails 桌面应用的单仓库。每个子目录都是一个完整项目，拥有自己的 Go 模块、Wails 配置、前端工程、测试和构建产物。
...
`RepoMirror`、`GistSync` 和 `logcat` 使用相同的 Go、前端和 Wails 检查流程。`GistSync` 的前端构建会额外执行 `vue-tsc`；`logcat` 运行桌面功能还需要本机 `adb`。
